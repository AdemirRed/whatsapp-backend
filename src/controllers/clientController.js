    ...
    switch (contentType) {
      case 'string':
        if (options?.media) {
          ...
          media.filename = null
          media.filesize = null
          safeOptions.media = new MessageMedia(media.mimetype, media.data, media.filename, media.filesize)
        }
        try {
...
const getProfilePictureUrl = async (req, res) => {
  ...
  */
  try {
    ...
    // Tenta o método moderno (requestProfilePicFromServer) e faz fallback para o método legado (profilePicFind)
    const result = await client.pupPage.evaluate(async (contactId) => {
      try {
        const chatWid = window.Store.WidFactory.createWid(contactId)
        let profilePic
        try {
          // Método moderno (versões >= 2.3000.0)
          profilePic = await window.Store.ProfilePic.requestProfilePicFromServer(chatWid)
        } catch (e) {
          // Fallback: método legado, não depende do contact.isNewsletter
          try {
  ...
  */
  try {
    ...
    try {
      ...
      // Se o erro for relacionado a markedUnread, tentar abordagem alternativa
      if (sendSeenError.message && sendSeenError.message.includes('markedUnread')) {
        console.log(`⚠️ Erro markedUnread em sendSeen, usando método alternativo...`)
        try {
          const chat = await client.getChatById(chatId)
          if (chat && typeof chat.sendSeen === 'function') {
            await chat.sendSeen()
            res.json({ success: true, result: 'Mensagem marcada como lida via chat.sendSeen()' })
          } else {
            throw new Error('Método chat.sendSeen não disponível')
          }
        } catch (altError) {
          throw new Error(`Não foi possível marcar como lido: ${altError.message}`)
        }
      } else {
        throw sendSeenError
      }
